...
**เวอร์ชันปัจจุบันของระบบ:** v0.7.33  
...
### 8.1 สถานภาพปัจจุบัน (ปัจจุบัน = v0.7.33)
...
| v0.7.28 | **แก้ "เลขไมล์ขากลับไม่ย้ายขึ้นชีต" (ตาม bug report ผู้ใช้ — รถกลับแล้วแต่สมุด บันทึกการเข้า-ออกรถยนต์ ยังโชว์เลขไมล์ขาออก):**
  ...
  - การตรวจ: `node --check` ผ่าน 9/9 |
| v0.7.29 | **แก้ "เครื่องอื่นเห็นเลขไมล์เป็นเลขไมล์เก่า" + "ข้อมูลตรวจสภาพเข้าไปเขียนในสมุดเข้า-ออก" + เพิ่มตรวจรุ่น GAS (ต้อง Redeploy GAS อีกรอบ!)**
  - **สาเหตุเชิงระบบ (2 ข้อ):**
    - **#1 ข้อมูลตรวจสภาพเข้าไปปิด/เขียนภารกิจในสมุด บันทึกการเข้า-ออกรถยนต์** = GAS ยังเป็นรุ่นเก่า (ไม่มี flag `completeDeparture`) → ทุก INSPECTION ปิดภารกิจโดยไม่ดูว่าใช่การกลับรถจริงไหม
    - **#2 เครื่องอื่นเห็นเลขไมล์เก่า** = GAS `VEHICLE` เขียนทับไมล์แบบไม่ตรวจ → เครื่องที่ข้อมูลไมล์เก่ากว่าดัน payload เก่าขึ้นสมุดกลางทับค่าใหม่ (ที่จริงแล้ว merge ฝั่งแอปกันไมล์ถอยหลังได้อยู่ แต่สมุดกลางถูก "ตีกลับเป็นเก่า" ตั้งแต่ต้นทาง ทำให้เครื่องที่ยังไม่เคยรับเลขไมล์ใหม่ เอาเลขไมล์เก่าไปใช้ตามสมุด) + ทางลัดขากลับยังถูก ignored ถ้า GAS เก่า
  - **แก้ (ฝั่ง GAS — ต้อง Redeploy):** เพิ่ม `GAS_BUILD` const + ส่งกลับในทุก response (ดู `gasBuild` ได้) + **กฎ "เลขไมล์ห้ามถอยหลัง" ต้นทาง GAS** (VEHICLE: กัน payload ไมล์ต่ำกว่าแถวปัจจุบัน, เขียนค่าสูงสุด + แก้ jsonFull ให้ตรง; DEPARTURE_END: เลขไมล์ขากลับห้ามถอยหลังเช่นกัน)
  - **แก้ (ฝั่งแอป v0.7.29):** จับ `gasBuild` ทุก response → แสดงในหน้าตั้งค่า Google Sheet + ปุ่ม **"ตรวจรุ่น GAS"** เปรียบเทียบ `GAS-v0.7.29`; ถ้าไม่ตรง = เตือน "ยังไม่ได้ Redeploy" (ไขข้อสงสัยที่เกิดซ้ำว่า GAS เก่าหรือใหม่ได้ทันที)
  - **การตรวจ:** `node --check` ผ่าน |
| v0.7.30 | **แก้ "แจ้งเตือนส่งซ้ำไม่หยุด" + "ขากลับบันทึกแล้วแต่เลขไมล์ master/เครื่องอื่นไม่เปลี่ยน" (ต้อง Redeploy GAS อีกรอบ! → GAS-v0.7.30)**
  - **#1 ส่งข้อความซ้ำไม่หยุด (Email Alert storm):** `_sendAlertEmail` ถูกเรียกจาก `checkFleetAlerts` ทุก ~2 นาที + ทุกบันทึกขากลับ; เมื่อส่ง FAIL จะ **ลบ claim** (`pea_alert_pm_cycle_*`) แล้วยิงซ้ำ-ล้ม-ยิงซ้ำ... วนไม่จบ → **เพิ่ม cooldown 30 นาทีต่อรอบ** (`pea_alert_cd_{cycleKey}_{baseline}`) stamp ก่อนลองส่ง (ครอบคลุมทั้งสำเร็จ/ล้มเหลว) — ต่อให้ล้มก็ต้องรอครบ 30 นาทีก่อนรอบถัดไป ไม่ส่งรัวอีก; รอบใหม่ (ค่า baseline เปลี่ยน) เริ่ม cooldown ใหม่ได้ไม่ตัน
...
| v0.7.32 | **แก้ "หน้าบันทึกขากลับ (Active Trips) มีข้อความ/ภารกิจค้างไม่หาย" (ต้อง Redeploy GAS อีกรอบ! → GAS-v0.7.32)** |
  ...
  - **เทสต์ไลฟ์ (หลัง user redeploy):** CHECK=GAS-v0.7.32; UPSERT ส่ง DEPARTURE ซ้ำ = 1 แถว; close-all ปิด 2 แถวพร้อมกัน master=5200 ✓
  - **การตรวจ:** `node --check` ผ่าน |
| v0.7.33 | **แยกข้อมูลไมล์ธุรกิจออกจากสมุดตรวจสภาพ (ต้อง Redeploy GAS อีกรอบ! → GAS-v0.7.33)**: ผลต่างระยะทาง/แจ้งเตือนไมล์เกิน/เติมน้ำมัน ย้ายไปสมุด "บันทึกการเข้า-ออกรถยนต์_Departures" (GAS คำนวณผลต่าง+แจ้งเตือนอัตโนมัติตอนปิดขากลับ) — สมุดตรวจสภาพเหลือ "ไมล์ก่อนตรวจ(กม.) + ผลการตรวจ + รายการจุดชำรุด" |
  - **สาเหตุ:** ผู้ใช้สั่งแยกข้อมูล — สมุดตรวจสภาพควรรับแค่หัวข้อตรวจสภาพ ส่วนไมล์/ผลต่าง/เตือน/เติมน้ำมันคือธุรกิจของสมุดเข้า-ออก; เพิ่มเติม: ขากลับรอบ 01:17 (01:17) ที่บันทึกจากเครื่องแคชเก่า/ออฟไลน์ไม่ถึง GAS → แถว MSN-MUBKIJ8I (PK-1109) ค้างไมล์ขากลับ
  - **แก้ (GAS GAS-v0.7.33):** inspection แก้ header+appendRow = 11 คอลัมน์ (ตัด ไมล์หลัง/ผลต่าง/แจ้งเตือน/เติมน้ำมัน); departure header+write = 14 คอลัมน์ (+ผลต่าง/แจ้งเตือน/เติมน้ำมัน ก่อนสถานะ) ทั้ง DEPARTURE, DEPARTURE_END และ INSPECTION-close; มี `migrateInspectionLayout()` (รีแมปตรวจสภาพเก่าไม่พัง) + `migrateDepartureLayout()` (ย้อนเติมผลต่าง/แจ้งเตือนให้แถวเก่า) รันกันเอง อ่านด้วย header ไทย → ปลอดภัย
  - **แก้ (แอป):** `submitReturnMileageModal` payload `UPDATE_DEPARTURE` เพิ่ม `mileageAlert` + `fuelRefill` (GAS เขียนลง 3 คอลัมน์ใหม่); การประกันไมล์ตอนนี้มาจากสมุดเข้า-ออก (depMaxById) แทนสมุดตรวจสภาพ
  - **ไล้ข้อมูลค้าง:** ปิด MSN-MUBKIJ8I (PK-1109) end=10100 → master PK-1109=10100; READ_ALL ยืนยันไม่มีแถว "ออกปฏิบัติงาน" เหลือ
  - **เทสต์ไลฟ์ (22/9, หลัง user redeploy):** CHECK=GAS-v0.7.33; migration ตรวจสภาพคอลัมน์ใหม่สะอาด + เข้า-อกแถวเก่าย้อนเติม (MSN-MUBKIJ8I ผลต่าง=80/ปกติ); INSPECTION ขาไปเขียนสะอาด; DEPARTURE_END ผลต่าง=40/ปกติ/เติม=20; delta 10960 → "เกิน 10,000 กม. (เฝ้าระวัง)" ✓
  - **ตรวจ:** `node --check` ผ่าน |
| v0.7.32 วางแผน | ตัวเลือกถัดไป: ส่งสรุปรายวัน/สัปดาห์, ปรับแต่ง HTML อีเมล, แจ้งเตือนจุดชำรุดวิกฤตถึงช่าง |

**สรุปงานวันนี้ (2026-09-22) — จบวัน พักตรงนี้ ทบทวนวันพรุ่งนี้**
**สิ่งที่ทำเสร็จวันนี้ (2 รอบ release + deploy + เทสต์ไลฟ์ผ่านหมด):**
- **v0.7.32** (`34b3b2c`): แก้ "Active Trips ค้างไม่หาย" — GAS DEPARTURE UPSERT ตาม missionId / DEPARTURE_END + INSPECTION-close ปิดทุกแถว / แอปรักษา mission.id เดิม + prefer completedById → กันแถวแฝด & ฟื้นคืนถาวร
- **v0.7.33** (`f13e1b5`): แยกข้อมูลไมล์ธุรกิจ — ย้าย ผลต่าง/แจ้งเตือนไมล์เกิน/เติมน้ำมัน ออกจาก "ตรวจสภาพ_Inspections" ไป "บันทึกการเข้า-ออกรถยนต์_Departures" (GAS คำนวณอัตโนมัติตอนปิดขากลับ) + migration 2 ตัว (รีแมปตรวจสภาพไม่พัง / ย้อนเติมเข้า-อกแถวเก่า) + แอปส่ง `mileageAlert`/`fuelRefill` ลง payload ขากลับ
- **ไล้ข้อมูลสมุด:** ปิดแถวค้าง MSN-MUBKIJ8I (PK-1109)=10100 / MSN-MUBG7PTG แฝด=10020 / MSN-MUBHGHTV (PK-2126)=10001 → **ไม่มีแถว "ออกปฏิบัติงาน" ค้างทั้งสมุด**
- **Heal master ไมล์ค้าง 4 คัน** (เครื่องเก่าเคยดันค่าไมล์ต่ำ): PK-4501→89500 / BK-3305→95260 / SD-1102→63120 / **SD-9908→20050** (เดิมค้าง 9000)
- **สถานะสมุดล่าสุด:** vehicles=14 / departures=19 / inspections=19; รถทดสอบ `TEST-DIAG-1/2` ยังมีในสมุด (ข้อมูล diag, ล้างได้ภายหลังถ้าไม่ต้องการ)
- **Commit/push:** `34b3b2c` (v0.7.32) → `f13e1b5` (v0.7.33) — สาขา `master` repo `BooK-PON/pea-smart-vehicle`

**งานต่อพรุ่งนี้ (TODO ลำดับ):**
1. **ผู้ใช้ Ctrl+F5 ทุกเครื่อง** → badge ต้องเป็น **v0.7.33** + "ตรวจรุ่น GAS" = **GAS-v0.7.33** (คันไหนยังขึ้นเก่า = แคชยังไม่ล้าง เตือนกด Ctrl+F5 ซ้ำ)
2. **เทสต์ workflow จริง 1 รอบเต็ม** (บนข้อมูลจริง): ตรวจสภาพก่อนออก → นำรถออก → เปิดอีกเครื่อง → กลับด้วยปุ่มลัด + กลับผ่านใบตรวจเต็ม (completeDeparture) → ตรวจสมุดว่า ไป/กลับ/ผลต่าง/แจ้งเตือน/เติมน้ำมัน เขียนถูก + สมุดตรวจสภาพเก็บเฉพาะเนื้อหาตรวจ
3. **เทสต์ใบรับรอง A4/PDF** ว่า ไมล์ไป/กลับ/ผลต่าง/เติมน้ำมัน ยังแสดงถูกหลังแยกสมุด (reportGenerator ใช้ค่าจากฟอร์มตอนพิมพ์ → ควรปกติ, เผื่อ verify)
4. **เตรียมเครื่อง** ที่บันทึกขากลับ 01:17 (แคชเก่า/ออฟไลน์) → เปิดเน็ต/กดปุ่มซิงค์ → คิว `UPDATE_DEPARTURE` จะ flush ขึ้นเป็น Completed ที่ 10100 ทับถูกต้อง (upsert กันแถวแฝด) — ยืนยันกับผู้ใช้
5. **ของค้างจาก "v0.7.32 วางแผน":** ส่งสรุปรายวัน/สัปดาห์, ปรับแต่ง HTML อีเมล, แจ้งเตือนจุดชำรุดวิกฤตถึงช่าง (ยังไม่เริ่ม — ผูกกับ `sendTestAlertEmail`/`checkFleetAlerts` เป็นต้นแบบ)
6. *(optional)* เก็บกวาดแถวทดสอบ `TEST-*`/`MSN-DIAG-*` ในสมุดออกให้สวย (ก่อนใช้งานจริงระยะยาว)
